...
export function VoiceSelector() {
  ...
  return (
    <Field>
      <FieldLabel>Estilo de voz</FieldLabel>
      <Select
        value={voiceId}
      ...
      >
        ...
        <SelectContent>
          {hasMissingSelectedVoice && currentVoice && (
            <>
              <SelectGroup>
                <SelectLabel>Voz seleccionada</SelectLabel>
          ...
          {customVoices.length > 0 && (
            <SelectGroup>
              <SelectLabel>Voces custom</SelectLabel>
              {customVoices.map((v) => (
          ...
          {systemVoices.length > 0 && (
            <SelectGroup>
              <SelectLabel>Voces del sistema</SelectLabel>
              {systemVoices.map((v) => (
                <SelectItem key={v.id} value={v.id}>
                  <VoiceAvatar seed={v.id} name={v.name} />
                  <span className="truncate text-sm font-medium">

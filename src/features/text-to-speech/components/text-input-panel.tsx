...
import { COST_PER_UNIT, TEXT_MAX_LEGTH } from "../data/constants";
    ...
    <div className="flex h-full min-h-0 flex-col flex-1">
      ...
      <div className="shrink-0 p-4 lg:p-6">
        ...
        <div className="flex flex-col gap-3 lg:hidden">
          <GenerateButton className="w-full" disabled={isSubmitting} isSumitting={isSubmitting} onSubmit={() => form.handleSubmit()} />
        </div>
        {/* Desktop layout */}
        {text.length > 0 ? (
            ...
            <div className="flex items-center gap-3">
              <p className="text-xs tracking-tight">
                ...
                <span className="text-muted-foreground">
                  &nbsp;/&nbsp;{TEXT_MAX_LEGTH.toLocaleString()} caracteres
                </span>
              </p>
              <GenerateButton size="sm" disabled={isSubmitting || !isValid} isSumitting={isSubmitting} onSubmit={() => form.handleSubmit()} />
            </div>
          </div>
        ) : (
          <div className="hidden lg:block">
            <p className="text-sm text-muted-foreground">
              Empieza a escribir o pega tu texto arriba
            </p>
          </div>
        )}
      </div>
